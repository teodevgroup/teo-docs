...
```ts showLineNumbers
import { App } from '@teodevgroup/teo'
...
```rust showLineNumbers
use tokio::main;
use teo::prelude::{App, Response, Result, Error};
...
```ts showLineNumbers
import { App, Response } from '@teodevgroup/teo'
...
other data types. Let us know by firing an [issue](https://github.com/teodevgroup/teo/issues).
...
```rust showLineNumbers {4,5,32-46}
mod entities;

use tokio::main;
use teo::prelude::{App, Response, Result, Error, serve_static_files, teon};
...
async fn main() -> Result<()> {
    ...
    app.main_namespace().define_model_handler_group("Record", |group| {
        group.define_handler("alterCreatedAt", |teo: Teo, input: AlterCreatedAtInput| async move {
            if let Some(record) = teo.record().find_unique(teon!({
                "where": {
                    "id": input.id()
                }
            })).await? {
...
app.mainNamespace().defineHandler("static", (request: Request) => {
...
async def main():
    ...
    app.main_namespace().define_handler("echo", echo_handler)
    def static_handler(request: Request):
        path_args: StaticPathArguments = request.path_arguments()
        return serve_static_files("static", path_args["path"])
    app.main_namespace().define_handler("static", static_handler)
    def record_group(group: HandlerGroup):
        async def alter_created_at_handler(request: Request):
            input: AlterCreatedAtInput = request.body()
            teo: Teo = request.teo()
...
app.mainNamespace().defineHandler("echo", (request: Request) => {
    const pathArguments: EchoPathArguments = request.pathArguments()
    return Response.string(pathArguments.data, "text/plain")
})
app.mainNamespace().defineHandler("static", (request: Request) => {
    const pathArguments: StaticPathArguments = request.pathArguments()
    return serveStaticFiles("static", pathArguments.path)
})
app.mainNamespace().defineModelHandlerGroup("Record", (group) => {
    group.defineHandler("alterCreatedAt", async (request) => {
        const teo: Teo = request.teo()
        const input: AlterCreatedAtInput = request.body()
        const record = await teo.record.findUnique({
            "where": {
                "id": input.id
            }
...
async def main():
    ...
    app.main_namespace().define_handler("empty", empty_handler)
    def echo_handler(request: Request):
        path_arguments: EchoPathArguments = request.path_arguments()
        return Response.string(path_arguments["data"], "text/plain")
    app.main_namespace().define_handler("echo", echo_handler)
    def static_handler(request: Request):
        path_args: StaticPathArguments = request.path_arguments()
    ...
    def upload_handler(request: Request):
        input: UploadInput = request.body()
...
with it, let us know by joining our [Discord group](https://teodev.io/discord)
or [firing an issue](https://github.com/teodevgroup/teo/issues).
