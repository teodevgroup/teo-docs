...
import { contentBackgroundColor } from '../../../../shared/styles/theme'
...
async def main():
    app = App()
    def hello_handler(_request, _body, _teo):
        return Response.html("""
            <html>
                <head>
                    <title>Hello, Teo handlers</title>
...
<PythonDoc>
```python showLineNumbers
    def empty_handler(_request, _body, _teo):
        return Response.empty()
...
```ts showLineNumbers {1,2,5,20-23}
...
Now navigate to `/static/images/girl.jpg` in the browser, and you will see the 
...
app.mainNamespace().defineHandler("upload", (ctx) => {
    ...
    const extname = path.extname(input.file.filepath)
    const extension = extname ? `.${extname}` : ""
